...
// Config holds Kuaishou adapter configuration.
type Config struct {
	AppKey        string
	AppSecret     string
...
// Adapter implements the ChannelAdapter interface for Kuaishou IM.
type Adapter struct {
	cfg        Config
	channelID  string
...
// NewAdapter creates a new Kuaishou adapter.
...
func NewAdapter(cfg Config, channelID string, getToken func(ctx context.Context) (string, error)) *Adapter {
	return &Adapter{
		...
		getToken: getToken,
		sendURL:  "https://open.kuaishou.com/openapi/mp/developer/message/send",
	}
}

// InboundMsg represents a Kuaishou inbound webhook message.
...
// OutboundMsg is the JSON payload for Kuaishou IM send API.
...
// ChallengeRequest represents a Kuaishou webhook challenge verification request.
type ChallengeRequest struct {
	Challenge string `json:"challenge"`
}

// VerifyWebhook validates the Kuaishou webhook HMAC-SHA256 signature from the HTTP request.
func (a *Adapter) VerifyWebhook(ctx context.Context, r *http.Request) error {
	signature := r.Header.Get("X-Kuaishou-Signature")
	timestamp := r.Header.Get("X-Kuaishou-Timestamp")
	nonce := r.Header.Get("X-Kuaishou-Nonce")
...
// ParseInbound converts a Kuaishou JSON message into a StandardMessage.
...
// FormatOutbound converts a StandardMessage into Kuaishou IM API JSON payload.
...
// SendMessage delivers a formatted payload via Kuaishou IM send API.
func (a *Adapter) SendMessage(ctx context.Context, channelID string, payload []byte) error {
	...
	if result.ErrCode != 0 {
		return fmt.Errorf("kuaishou API error %d: %s", result.ErrCode, result.ErrMsg)
	}

	log.Printf("[kuaishou] message sent to %s", channelID)
	return nil
}

// parseContent extracts normalized MessageContent from a Kuaishou inbound message.
// The Content field in Kuaishou messages is a JSON string that requires double parsing.
...
// IsChallenge checks if the request body is a Kuaishou webhook challenge request.
