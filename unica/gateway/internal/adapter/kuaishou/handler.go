...
// WebhookHandler handles Kuaishou webhook HTTP requests.
type WebhookHandler struct {
	adapter  *Adapter
	producer *stream.Producer
}

// NewWebhookHandler creates a handler for the Kuaishou webhook endpoint.
...
// ServeHTTP handles incoming Kuaishou webhook requests.
// Kuaishou sends all webhooks as POST requests (both challenge and messages).
...
func (h *WebhookHandler) handlePost(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.adapter.VerifyWebhook(r.Context(), r); err != nil {
		log.Printf("[kuaishou] signature verification failed: %v", err)
	...
	if err != nil {
		log.Printf("[kuaishou] read body error: %v", err)
	...
	if challenge, ok := IsChallenge(body); ok {
		log.Printf("[kuaishou] webhook challenge received")
	...
	if err := json.Unmarshal(body, &msg); err != nil {
		log.Printf("[kuaishou] parse error: %v", err)
		w.WriteHeader(http.StatusOK)
	...
	if err != nil {
		log.Printf("[kuaishou] convert error: %v", err)
		w.WriteHeader(http.StatusOK)
		w.Write([]byte("success"))
		return
	}
	...
	if h.producer != nil {
		...
		if err != nil {
			log.Printf("[kuaishou] publish error: %v", err)
			w.WriteHeader(http.StatusOK)
			w.Write([]byte("success"))
			return
		}

		log.Printf("[kuaishou] message published: id=%s stream=%s from=%s type=%s",
