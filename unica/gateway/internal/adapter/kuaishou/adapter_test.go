...
// newSignedRequest creates an HTTP request with valid Kuaishou HMAC-SHA256 signature headers.
func newSignedRequest(t *testing.T, method, secret, body string) *http.Request {
	...
	req := httptest.NewRequest(method, "/webhook/kuaishou", bodyReader)
...
func newTestAdapter() *Adapter {
	...
	}, "ch_kuaishou_001", func(ctx context.Context) (string, error) {
...
func TestVerifyWebhook_Invalid(t *testing.T) {
	...
	req.Header.Set("X-Kuaishou-Signature", "bad_signature")
	req.Header.Set("X-Kuaishou-Timestamp", "123")
	req.Header.Set("X-Kuaishou-Nonce", "abc")
	if err := a.VerifyWebhook(context.Background(), req); err == nil {
...
func TestParseInbound_Text(t *testing.T) {
	...
	if msg.Source != "adapter.kuaishou" {
		t.Errorf("source = %q, want %q", msg.Source, "adapter.kuaishou")
	}
}
...
func TestParseInbound_InvalidJSON(t *testing.T) {
	a := newTestAdapter()
	req := httptest.NewRequest(http.MethodPost, "/webhook/kuaishou", strings.NewReader("not json"))
...
func TestParseInbound_AllFieldsPreserved(t *testing.T) {
	...
	if msg.Data.ChannelID != "ch_kuaishou_001" {
...
func TestSendMessage_Success(t *testing.T) {
	...
	err := a.SendMessage(context.Background(), "ch_kuaishou_001", payload)
	if err != nil {
...
func TestSendMessage_APIError(t *testing.T) {
	...
	err := a.SendMessage(context.Background(), "ch_kuaishou_001", []byte(`{}`))
	if err == nil {
