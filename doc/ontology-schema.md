...
| 0.75 | 注入了事实，未获得校验加成（没做出可校验的断言，或断言未全部成立） |
| 0.72 | 只召回了历史经验（启发式证据，弱于确定性事实） |
| 检索均值 | 未注入任何上下文时的原有行为，一分不变 |

**冲突不降分。** 与本体冲突的回答按"未校验"计分，压制只发生在 `enforce`
的强制转人工里，受熔断约束。若把冲突折进分数，压制会借 `low_confidence`
通道发生：熔断拦不住它，`shadow` 模式也会因此改变路由，还会把冲突误记为
失败经验样本——三条都违反各自机制的书面承诺。
...
本体里写错一条断言，**每一条碰到它的正确回答都会被压下转人工**——错误规模随流量增长，
而且没有预警。这不是假设：实测的三条产品线里有两条第一次写就带着这种缺陷
（加急时效存成「120分钟」而话术是「2小时」、退货流程压根没声明），
黄金集当场抓到了，线上没有黄金集。

所以 `enforce` 是有上限的。滑动窗口内被压下的比例超过阈值，就**自动停止拦截并告警**：

```json
{
  "ontology": {
    "validation": "enforce",
    "breaker": { "trip_rate": 0.25, "min_samples": 20, "window": 100, "cooldown_seconds": 900 }
  }
}
```
...
| 字段 | 默认 | 含义 |
|---|---|---|
| `trip_rate` | `0.25` | 最近窗口内被压下的比例达到此值即熔断 |
| `min_samples` | `20` | 少于这么多样本不作判断，避免冷门产品线一条误判就停 |
| `window` | `100` | 统计最近多少条已校验回答 |
| `cooldown_seconds` | `900` | 停止拦截多久后重新评估 |
| `disabled` | `false` | 关掉熔断，让 `enforce` 无上限 |

**整块 `breaker` 可以不写，默认就是开的**——它只会放松你主动开启的拦截，不会收紧任何东西。

几点需要清楚：

- **熔断态不是"安全态"**，是另一种风险：不拦截意味着与本体冲突的回答会发给客户。
  它换掉的是"人工队列被瞬间打爆"。代价用 `router_ontology_breaker_bypassed_total` 明确计数，不藏着。
- **`shadow` 期间窗口一样在统计**。所以一条产品线如果比例已经离谱，
  切到 `enforce` 的第一条消息就会被熔断挡住，不会先压一批回答再发现。
- **冷却结束后按冷却期间的数据重新判断**。本体还是错的就继续不拦，
  而不是"时间到了再拦一批"。修好本体、比例掉下去，才会自己恢复。
- **状态是每进程的**，不走 Redis：安全装置不该依赖一个出事时可能一起挂掉的组件。
  代价是 N 个副本各判各的，临界比例时可能有的副本在拦有的没拦。

监控：`router_ontology_breaker_open{product_line}`（1 = 当前未拦截）、
`router_ontology_breaker_trips_total`（涨了就说明本体有问题，要去看，不是调阈值）、
`router_ontology_breaker_bypassed_total`（明知冲突仍发出去的回答数）。
