# Active Task: AI 设置页收拢 S1–S11（完成）
...
## Current Status
- [x] S1–S11 全部完成（2026-08-24）

### S11 —— 先造仪器，再验假设，最后才给控件

计划把 top_k 排在最后，理由写得很直白：**"single/router 模式下取数据集 top_k"目前只是假设，
仓库两次栽在同类假设上**。所以顺序是固定的——先让测试消息能回显召回，
用它证明 top_k 真的改变召回，然后才做成控件。

**一、仪器。** 测试消息现在回显：召回条数、每一段的来源文档、相似度、以及正文前 120 字。
零段是最要紧的那种情况——**没用到知识库的回答，和用到了知识库的回答，读起来一模一样**，
在此之前页面上没有任何地方能把这两者分开。

admin 的桥此前只从 `metadata.retriever_resources` 里取一个 score，文档名、数据集名全丢了；
而 Dify 在不同版本里把这个字段放在两处（顶层 / metadata 下），只读一处的后果是
**给一个正常工作的知识库报"0 段"**。两处都读了。

**二、假设验证。** 拿这台仪器实测（XDYX，43 篇文档，strategy=router / retrieval_model=single）：

```
写入 top_k=2  回读 2  -> 测试消息召回 2 段
写入 top_k=8  回读 8  -> 测试消息召回 8 段
写入 top_k=6  回读 6  -> 测试消息召回 6 段
```

**假设成立**，而且是从控制台自己看得见的，不是靠推理。

**三、控件。** `PUT .../ai-settings/dataset/top-k`，仅 admin，1–10，拒 0
（0 会被运行时读成"没设过"并替换成它自己的默认值——和置信度阈值同一个坑）。
上限 10 是判断：再往上是把模型喂到记不住，把正确答案埋进一堆近似段落里，而不是找到更多。

**同时补上一个会互相回滚的结构**：`GetDatasetConfig` 现在把数据集当前的 top_k
读进 `Overrides`。PATCH 替换的是整个 `retrieval_model` 对象，
不这么做的话「修复检索」每跑一次就会把管理员设的值冲回平台默认，
而两个按钮谁都不会报错。实机验过：设 9 之后跑一次修复，仍然是 9。

**一条旧单测因此改写。** 它原本证明"只应用了 search_method 的 PATCH 会被抓到"，
用的是"库里存着 99、平台默认是 6"的场景。这个场景现在不存在了——
修复会主动把 99 带上，压根不要求改它。改成测真正还会静默失效的那件事：
**管理员显式设的 top_k 被 Dify 丢掉**。另加一条：修复不得回滚管理员的值。

### 过程中修掉两个挡路的缺陷

**一、admin 的 10 秒写超时**。测试消息是这一页唯一一个故意去等大模型的接口，
而服务器的写超时是 10 秒。一个耗时 31 秒的问题会被**中途掐断连接**，
操作者看到的是"网络错误"而不是"回答比较慢"——**用来判断其他一切的那台仪器，
自己看起来是坏的**。改成与上传同样的做法：用 `ResponseController` 把这个请求的
读写截止时间放宽到 3 分钟，并给请求上下文同样的窗口（否则连接断了活还在后台跑）。

**二、Dify 桥只有一个 30 秒的 http 客户端**。控制台调用与聊天调用共用它，
于是"控制台调用不该超过 30 秒"这条要求悄悄管住了"聊天调用可以更久"这件事，
31 秒的问题变成传输错误。拆成两个客户端：控制台仍是 30 秒，
聊天客户端不设自己的超时，由调用方的 context 决定（留 5 分钟兜底防止无 deadline 的调用者挂死）。

### 顺带看见的一件事（不在本轮处理）

top_k 现在可见了，于是能一眼看出：**五条演练线的数据集 top_k 都是 2**——
那是 Dify 自己的默认值，说明它们的检索设置从来没有对齐过平台默认（6）。
在营的 XDYX / AJYJ 是 6，另外三条零售线根本没有数据集（D8）。演练线不影响任何人，登记备查。
...
**前置验证。** 计划写着"先在真实 Dify 上验证写进去再读回来字节完全相同，
不成立则改用对规范化不敏感的比较"。验了两轮：六条产线的模板全部 sha256 一致；
又专门写了一段**行尾空格、制表符、连续空行、零宽字符、ZWJ 组合 emoji、400 字长行**
的文本，写回读出仍然逐字相同。前提成立，可以用哈希比。

**但比什么，是这一步真正的问题。** "和当前模板不一样"是两种完全不同的情况，
而它们长得一模一样：一种是租户自己改的（不该去动它），另一种是**被模板更新落下的**
（从来没人告诉过他）。后者就是 D16，它之所以一直不可见，正因为它和前者无从区分。

所以判据不是"比模板"，而是**记住控制台每次写了什么、以及当时它是不是平台模板**
（`config_json.prompt_origin`）。据此五种状态，每一种都拿得出证据：

| 状态 | 依据 |
|---|---|
| 与平台模板一致 | 线上正文 == 当前模板（**不需要记录**，所以存量线天然为真） |
| 平台模板已更新，本产线仍是旧版 | 记录说"当时写的就是平台模板"，而线上正文仍等于它 |
| 已自定义 | 记录说"写的是租户自己的文本"，线上正文仍等于它 |
| 控制台之外被改过 | 线上正文 ≠ 记录里的哈希——多半有人直接在 Dify 里改了 |
| 来源未知 | 没有记录，早于本功能。不猜，下次保存或恢复即自愈 |

**存量线的记录是补上的**：今天全部产线都逐字等于当前模板，这是**今天知道、
明天就找不回来**的事实——模板一改就退化成"来源未知"。所以当场用「恢复平台模板」
把同样的字节重写了一遍（已验证逐字不变），把记录落下。

**验收就按计划做了真的改模板**：把平台模板改一个词、重新编译部署 admin：

```
AJYJ / FreshMart / MegaStore / TechZone / XDYX / DrillCo / DrillCo5   平台模板已更新，本产线仍是旧版
DrillCo4（事先写过自己的文案）                                        已自定义 ← 不误报
DrillCo2 / DrillCo3（无记录）                                        来源未知
```

改回模板、重新部署后，五条在营线的长度与状态与验收前逐字一致。

### 顺带修掉一个独立缺陷：新租户的 AI 自称"UNICA-XXX"

验证过程中开了个一次性租户，发现它的提示词首行是
**"你是UNICA-ORIGINTEST的在线客服"**。`CreateChatApp` 把一个参数同时当成了
Dify 应用名和产线名，而应用名带着本部署的 `UNICA-` 前缀——**平台自己的开户命名规范，
被讲给了客户听**。它还让每个新租户一开户就与平台模板不一致（控制台写模板用的是产线名），
于是漂移判定对新户永远是"来源未知"。

已拆成两个参数：应用名给 Dify 控制台列表，产线名给提示词。
实机复验：新开租户首行为"你是ORIGINTEST2的在线客服"，对齐状态 `current`。
