...
## Current Status
- [x] S1–S8 完成（2026-08-24）

### S8 —— 提示词契约、权限反转、审计补全

**先验证再落地。** 计划自己的风险一节写着"S8 若把契约清单定错，会挡住所有租户的
提示词保存"，所以第一步不是写校验，是拿五条在营产线的**线上提示词**去对拍拟定的清单：

```
AJYJ / FreshMart / MegaStore / TechZone / XDYX   六项全部满足
DrillCo / DrillCo2 / 4 / 5                      缺 {{scene_context}}、[HANDOFF:
DrillCo3                                        缺 [HANDOFF:
```

五条在营线一条都不会被挡；被挡住的四条演练线，恰恰就是 **D16 说的"停在旧模板上的
存量租户"**——它们真的在用一个转人工标签协议不存在的提示词跑。

**契约是六项**：四个占位符（事实 / 参考知识 / 应答策略 / 历史经验）加两个标签协议
（`[FACT:`、`[HANDOFF:`）。判据统一：**缺了它会不会静默失效**。
六项全都会——检索照常命中、指标照常自增、违规记录页照常空着，
而模型什么都没收到。清单连同"缺失后果"一起放在 `pkg/difyapp`，
门户不留副本（`GET /ai-settings` 把整份契约一起返回），
否则两边迟早各说各话，而门户是看不出分歧的那一边。

**权限反转。** 原来是"覆盖不需要权限、恢复需要 admin"——守卫装在唯一只会
把产线**推向平台文本**的操作上，却对唯一能把它改坏的操作完全放行。
现在守卫在写入侧（契约不全直接拒），「恢复平台模板」对租户开放。
实机验收（DrillCo）：契约缺 2 项 + 变量缺 1 项 → **一次点击后两者都齐全**。

**过程中被单测拦下的一次多余改动**：我担心"模板写进去了但应用没声明对应输入，
占位符会原样留在模型读到的文本里"，于是在恢复流程里加了一次 `EnsureContextVariables`。
`TestResetPrompt_RefusesAdvancedPromptMode` 立刻红了——它跑在提示词写入**之前**，
于是一个注定要被拒绝的恢复（advanced 模式）反而先改了应用。
调到写入之后，另一条测试又红：`UpdateSystemPrompt` **本来就在同一次
model-config 写入里声明了这些变量**。整个改动是多余的，而且它引入了
对同一个对象的第二次读-改-写——两次全量写抢一个文档，输的那次会顺手把提示词带回旧值。
已删除。这是我这轮唯一一次没先查就动手。

**审计。** 快照原先只覆盖 config_json，而提示词存在 Dify，
于是**改动最大的那个操作留下的是一行 before/after 完全相同的记录**。
现在带上 sha256 + 字数 + 契约是否完整，**不抄正文**——审计表不该变成
每个租户历史提示词的第二份存储。Dify 读不到时降级为 `unavailable`，
不牵连快照的其余部分：审计失败永远不该成为"这次写入没被记录"的原因。

**体检卡顺带修掉三处假绿。** 提示词丢了 `{{knowledge_context}}` 时，
检索照常跑、命中数照常统计，而这一行此前显示"已接通"。现在它说
"检索得到，但送不进模型"。事实注入与应答策略同理。

**实机验收**：

```
六项逐一删除          全部 400 且指名是哪一项；线上提示词逐字未变
合规改写              200，审计 before/after 两个 hash 不同
审计正文              无提示词内容
清理                  XDYX 提示词已还原，与验收前逐字一致
门户                  编辑时实时标红；缺项时保存被拦；覆盖前显示行级 diff
                      契约不全的产线，卡片 pill 由绿色「已同步」改为「契约缺 N 项」
```
