...
最后更新：2026-09-03（配置面重构 B 组验收期间发现）
...
## D16 提示词模板改进从不流向存量租户，只有新开户才拿得到（已解，2026-08-25）
...
**2026-08-24 缩窄了一半**：回推路径本身此前是 admin 专属的，租户自己够不着；
现已对租户开放（按钮改名「恢复平台模板」），一次点击同时补齐提示词与它引用的输入声明，
实机在 DrillCo 上验过（契约 2 项缺失 + 变量 1 项缺失 → 一次点击后两者齐全）。
同时提示词写入加了**契约硬校验**：漏掉 `{{facts_context}}` 等六项之一即拒绝保存，
门户在编辑过程中就标红，体检卡也会因此把对应链路判为断开
（`pkg/difyapp` 的 `PromptRequirements`）。

**2026-08-24 另一半也补上了**：`config_json.prompt_origin` 记下控制台每次写入的内容
及当时是否等于平台模板，据此把"和模板不一样"拆成**已自定义**与**被模板更新落下**
（还有"控制台之外被改过"和"无记录"两种诚实的状态）。存量线的记录已当场补齐——
今天它们逐字等于当前模板，这是明天就找不回来的事实。
验收按计划改了一个词的平台模板重新部署：七条对齐过的产线报"仍是旧版"，
事先写过自己文案的那条报"已自定义"（不误报），无记录的报"来源未知"。
门户在提示词卡顶部给出提示与「更新到最新模板」按钮。

**2026-08-25 解掉**：提示词的权威源从 Dify 搬回 UNICA（配置面重构阶段 2）。
`prompt_versions` 表存正文与版本，Dify 降为投影；每次写入是"先定版、后投影"两段，
两段之间的「已定版未生效」是一个有列（`pushed_at IS NULL`）、有接口字段（`pushed`）、
有界面文案的真实状态，而不再是看不见的失败。平台侧多了跨租户落后清单
（`GET /api/v1/platform/prompts`）与显式勾选的回推（`POST .../push`），
租户侧多了版本历史与回滚——**误保存第一次可以退回租户自己的旧文案**，不再只能退回平台模板。

实机走完（本地 WSL，10 条产线）：改平台模板一个字重新部署，**恰好那 8 条原本一致的转为落后，
两条自己写过文案的没有误报**；勾选两条回推，只有它们转绿、其余 6 条与 2 条自定义纹丝不动；
回滚后 Dify 正文**逐字节**回到旧值；演练前后 10/10 正文完全复原。
搬迁本身只读不写：`promptaudit` 默认出差异报告 + 全量备份，`--seed` 才把现行正文落成 v1，
Dify 侧零写入（前后哈希 10/10 相同）。

**下面这段是修复前的原文，保留以说明当时的状态**：没有任何东西会**告诉**存量租户平台模板更新了。
契约校验只保证"不缺关键接口"，不保证"和当前模板同版本"——
一条停在旧模板但六项俱全的产线，页面上一切正常。这正是 S9 要做的漂移提示，
前提是先验证 pre_prompt 经 Dify 读-改-写后字节稳定（否则会永久误报"已自定义"）。
...
## D24 三条演练线的知识库索引与检索对不上（存量数据，非代码缺陷）

B 组的知识库清单上线当天就把它照了出来（2026-09-03，预览环境）：
**三个数据集的索引方式是 `economy`，检索方式却是 `semantic_search`**——
两条「演练客户」线与一条「界面演练客户」线。
...
这正是计划文档第四条点名的那个病（`plan-workbench-settings.md` 第 126-131 行，
上一轮在 DrillCo2/3/5 上抓到过）。成因是历史上改过 `DIFY_INDEXING_TECHNIQUE`：
改动只影响此后新建的文档，存量库的检索设置留在原处，于是同一个部署里
同时存在两种索引方式的库。

**不是代码缺陷**，是存量数据的状态，所以修法在数据侧：把这三个库的检索方式
改成 `keyword_search`，或者重建索引。B4 的清单现在会把这种库标红并写明后果
（`portal/admin.html` 的索引清单，`selfConsistent` 分支），
因此它不再是看不见的——但它仍然坏着，直到有人去动那三个库。

---

## D25 `pkg/domain` 的本体与话术种子已漂移（既有，本组未碰）

`go test ./pkg/domain/` 的 `TestSeedOntologiesMatchCannedResponses` 失败：
FreshMart / MegaStore / TechZone 三份种子本体里的「分场景必问项」「转人工采集通则」
「跳过采集的例外情形」等条目，在 `canned_responses.yaml` 里找不到对应话术。
测试自己给出的判词是「本体与 canned_responses.yaml 已经漂移」。

2026-09-03 B 组验收时发现。**与本组无关**：`git status` 确认 `pkg/domain` 与
那份 yaml 本组一行未改，是先前就红着的。其余四个模块（router / admin / gateway / reporter）
全绿，只有 `pkg/domain` 这一个包这一条失败。

后果目前只在种子数据上：新开的租户拿到的本体条目里，有一部分没有配套话术。
修的方向是两份文件对齐——要么补话术，要么把没有话术的本体条目删掉；
但该由谁对齐、以哪份为准，需要先定，所以先登记不动。
